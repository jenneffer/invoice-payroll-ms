    ...
    <div class="card-body">        
        <form id="payrollFormUpdate" name="payrollFormUpdate">
            ...
            <div class="row form-group">
                ...
                <div class="col-sm-2">                    
                    <input type="text" id="payroll_date" name="payroll_date" class="form-control date" value="{{ old('payroll_date', ($employee_data !==null) ? $employee_data->payroll_date : '') }}"  required>
            ...
            <table id="job_details" class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-right"><span id="gTotal">{{ number_format(($total_salary + ($employee_data->allowance ?? 0)) - (($employee_data->deduction ?? 0)+($employee_data->emp_tax??0)),2)}}</span></td>
                        <td>&nbsp;</td>
                    </tr>
                </tbody>
            </table>
            <div class="row col-sm-12">
                    <a class="btn btn-default" href="{{ route('admin.timesheet.index') }}">
...
$(document).ready(function(){      
    ...
    $(document).on('click', '.edit_data_payroll', function(){
        ...
        $.ajax({
            ...
            success:function(data){                
                ...
                $('#totalSalary').text(data.total.toFixed(2));
                $('#date').val(data.date);                   
                $('#job_name').val(data.job_id); 
                $('#time_start').val(data.time_start);  
                $('#time_end').val(data.time_end);  
                $('#rest_time').val(data.time_rest);   
                $("input[name='total_hours']").val(total_hours);     
                $("input[name='total_bin']").val(total_bin);   
    ...
    $( "#delete_record_payroll_details" ).click( function() {
        var ID = $(this).data('id');               
        $.ajax({
            url:"/admin/timesheet/details/destroy",
            method:"POST",
            data:{
                "_token": "{{ csrf_token() }}",
                id:ID,  
                payroll_id: '<?= json_encode($timesheet->id); ?>'               
            },
    ...
    $('#update_payroll_details').on('submit',function(event){           
        ...
        $.ajax({
            url: "/admin/timesheet/details/update",
            type:"POST",               
            data:{
                "_token": "{{ csrf_token() }}",
                data: payrollDetails,  
