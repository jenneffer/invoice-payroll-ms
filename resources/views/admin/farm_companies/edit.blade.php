...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route("admin.farm_companies.update", [$farm_company->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')            
            <div class="form-group {{ $errors->has('comp_name') ? 'has-error' : '' }}">
                <label for="comp_name">Farm Company Name *</label>
            ...
            <div class="form-group {{ $errors->has('comp_address') ? 'has-error' : '' }}">
                <label for="comp_address">Farm Company Address *</label>
            ...
            <div class="form-group {{ $errors->has('super') ? 'has-error' : '' }}">                            
                ...
                <input name="super" type="checkbox" value="1" {{ ($farm_company->super == 1 ? 'checked' : '')}}>
                @if($errors->has('super'))
                    <em class="invalid-feedback">
                        {{ $errors->first('super') }}
                    </em>
